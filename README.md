# 8 月 22 日停更通知：
## 由于 Apple 对 Apple 账户认证做出了非常严苛的调整和修改，已经无法通过此 App 登陆 Apple 账户/获取旧版本 IPA 了。目前没有变通方法，同类型 App 也遇到了类似问题。
## 如果 Apple 后续对此调整没有做出改变或放宽限制，此 App 会无限期停更。
## 同类型 App 目前也不再能使用 (如果还能使用，则表示会话没有超时，一旦超时则再也无法登陆)。例如 ipatools、Asspp 等。
